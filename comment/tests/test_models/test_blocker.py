...
class BlockerManagerTest(BaseBlockerManagerTest):
    ...
    def test_get_blocked_user_by_user_id_when_already_exists(self):
        blocked_user, created = BlockedUser.objects.get_or_create_blocked_user_by_user_id(self.blocked_user.id)
        self.assertFalse(created)

    def test_create_blocked_user_by_user_id(self):
        blocked_user, created = BlockedUser.objects.get_or_create_blocked_user_by_user_id(self.unblocked_user.id)
        self.assertTrue(created)

    def test_get_blocked_user_by_email_when_already_exists(self):
        blocked_user, created = BlockedUser.objects.get_or_create_blocked_user_by_email(self.blocked_email)
        self.assertFalse(created)

    def test_create_blocked_user_by_email(self):
        blocked_user, created = BlockedUser.objects.get_or_create_blocked_user_by_email('test@test.com')
        self.assertTrue(created)
