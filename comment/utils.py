...
def can_moderate_flagging(user):
    return bool(
        settings.COMMENT_FLAGS_ALLOWED and
        (is_comment_admin(user) or is_comment_moderator(user))
    )


def can_moderator_block_users(moderator):
    return settings.COMMENT_ALLOW_MODERATOR_PERFORM_BLOCKING and is_comment_moderator(moderator)


def can_block_user(user):
    return bool(
        settings.COMMENT_ALLOW_BLOCKING_USERS and
        (is_comment_admin(user) or can_moderator_block_users(user))
    )
